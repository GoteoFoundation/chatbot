...
@section('content')
  <div class="col-sm-8 offset-sm-2 mb-4">
    ...
    <h1 class="display-3">@lang('Edit language') {{ $language->id }}</h1>
    <div>
      @if ($errors->any())
      ...
      <form method="post" action="{{ route('languages.update', $language->id) }}">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control w-25" id="iso_code" name="iso_code" value="{{ old('iso_code', $language->iso_code) }}" maxlength="2" minlength="2" required>
        </div>
          <div class="form-group">
        ...
        @unless($copies->isEmpty())
          ...
          @foreach($copies as $key => $value)
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="{{ $key }}" name="{{ $key }}" value="{{ old($key, $language->getCopy($key)) }}" required>
            </div>
          @endforeach
        @endunless
...
@push('js')
  ...
  <script>
    goteoData = {
      ...
      selectedOption: "{{ old('support_language', $language->language_id) }}",
      translations: {
