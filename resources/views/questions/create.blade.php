...
<div class="py-4">
  <div class="col-sm-8 offset-sm-2">
    ...
    <h1 class="display-3">@lang('Add question')</h1>
    <div>
      @if ($errors->any())
      ...
      <form id="editQuestionForm" method="post" action="{{ route('topics.questions.store', $topic->id) }}">
        @csrf
        ...
        <div id="answers" class="form-group">
          ...
          <div id="items" data-repeater-list="answers">
                        ...
                        <div class="tab-content" id="answers-tabs-tabs-content-id1">
                          @foreach($languages as $lang)
                            <div class="tab-pane{{ $lang->is_parent ? ' show active' : '' }}" id="answers-{{ $lang->iso_code }}-id1" role="tabpanel" aria-labelledby="answers-tab-{{ $lang->iso_code }}-id1">
                              <input type="text" class="form-control" name="answers[text-{{ $lang->iso_code }}][]" value="{{ old($actual_field . '.' . $lang->iso_code) }}" {{ $lang->is_parent ? 'required' : '' }}>
                            </div>
                          @endforeach
                        </div>
                      </div>
                          ...
                          <div class="tab-content" id="answers-tabs-tabs-type-content-id1">
                            @foreach($languages as $lang)
                              <div class="tab-pane{{ $lang->is_parent ? ' show active' : '' }}" id="answers-type-{{ $lang->iso_code }}-id1" role="tabpanel" aria-labelledby="answers-tab-type-{{ $lang->iso_code }}-id1">
                                <input type="url" class="form-control" name="answers[url-{{ $lang->iso_code }}][]" value="{{ old($actual_field . '.' . $lang->iso_code) }}">
                              </div>
                            @endforeach
                          </div>
                        </div>
            ...
            @if(old('answers') && is_array(old('answers')))
                              ...
                              <div class="col-8">
                                <select class="form-control select2-questions" name="answers[question][]" style="width: 100%;" {{ $answer['type'] == 'question' ? 'required' : '' }}>
                                  ...
                                  @foreach($questions as $question)
                                    <option value="{{ $question->id }}" {{ ($answer['question'] ?? '') == $question->id ? 'selected' : '' }}>{{ $question->question }}</option>
                                  @endforeach
                                </select>
                              </div>
                              <div class="col-4">
                              ...
                              <div class="tab-content" id="answers-tabs-tabs-type-content-id{{ $itemsOnPage }}">
                                @foreach($languages as $lang)
                                  <div class="tab-pane{{ $lang->is_parent ? ' show active' : '' }}" id="answers-type-{{ $lang->iso_code }}-id{{ $itemsOnPage }}" role="tabpanel" aria-labelledby="answers-tab-type-{{ $lang->iso_code }}-id{{ $itemsOnPage }}">
                                    <input type="url" class="form-control" name="answers[url-{{ $lang->iso_code }}][]" value="{{ $answer['url-' . $lang->iso_code] }}" {{ ($lang->is_parent && ($answer['type'] == 'url')) ? 'required' : '' }}>
                                  </div>
                                @endforeach
                              </div>
                            </div>
        ...
        <button id="editQuestionFormButton" type="submit" class="btn btn-primary">@lang('Save')</button>
      </form>
    </div>
  </div>
</div>
...
@push('js')
  ...
  <script>
    goteoData = {
      ...
      topicId: "{{ $topic->id }}",
      isEmpty: {{ old('answers') && is_array(old('answers')) ? 'false' : 'true' }},
